...
void Analyzer_tautau::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
   ...
   /*//these are the fake factor files right here
   TFile* ff_file = TFile::Open("fakeFactors.root");
   ...
   */
   ...
   TFile* ff_file_my_alt = TFile::Open("fake_factors.root");
   TH1F* ff_mine_0 = (TH1F*)ff_file_my_alt->Get("ratio_decay_0");
   TH1F* ff_mine_1 = (TH1F*)ff_file_my_alt->Get("ratio_decay_1");
   TH1F* ff_mine_10 = (TH1F*)ff_file_my_alt->Get("ratio_decay_10");
   ...
   for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++) {
      ...
      tau2Cand.clear();
      int skip_me = -1;
      tau1Cand = getTau1CandTight(55, 2.1);
      if (tau1Cand.size()>0) skip_me=tau1Cand[0];
      tau2Cand = getTau2CandFail(45, 2.1, skip_me);
      dont_double_count = -1;
	...
	// trigger
	if( (HLTEleMuX>>29&1) == 1 || (HLTEleMuX>>30&1) == 1 || (HLTEleMuX>>31&1) == 1 ){
            nSingleTrgPassed+=event_weight;
            
            // first tau
            if (tau1Cand.size()>0){
	      event_weight = event_weight*0.89; //Single tau ID
		  ...
		  event_weight = event_weight*tauSFs.getTriggerScaleFactor(tauPt->at(tau1Cand[0]), tauEta->at(tau1Cand[0]), tauPhi->at(tau1Cand[0]), tauDecayMode->at(tau1Cand[0]));
		  event_weight = event_weight*tauSFs.getTriggerScaleFactor(tauPt->at(tau2Cand[0]), tauEta->at(tau2Cand[0]), tauPhi->at(tau1Cand[0]), tauDecayMode->at(tau2Cand[0]));
		  ...
		  /*
                  inputs1[0] = tauPt->at(tau2Cand[0]);
                  inputs1[1] = tauPt->at(tau1Cand[0]);
                  inputs1[2] = tauDecayMode->at(tau1Cand[0]);
                  inputs1[3] = nJet;
                  inputs1[4] = VisMass_F(myTau1, myTau2);
                  inputs1[5] = 1;
                  inputs1[6] = 0;
                  inputs1[7] = 0;

                  ff_nom = ff1->value(inputs1);
                  // event_weight = event_weight * 0.5 * ff_nom;
		  */
		  event_weight = event_weight * prefiring_weight(prefiring_photon, prefiring_jet);

                  float my_value = 0;
                  if (tauDecayMode->at(tau1Cand[0]) == 0) my_value = ff_mine_0->GetBinContent(ff_mine_0->GetXaxis()->FindBin( tauPt->at(tau1Cand[0]) ) );
                  if (tauDecayMode->at(tau1Cand[0]) == 1) my_value = ff_mine_1->GetBinContent(ff_mine_1->GetXaxis()->FindBin( tauPt->at(tau1Cand[0]) ) );
                  if (tauDecayMode->at(tau1Cand[0]) == 10) my_value = ff_mine_10->GetBinContent(ff_mine_10->GetXaxis()->FindBin( tauPt->at(tau1Cand[0]) ) );

                  event_weight = event_weight * 0.5 * my_value;

                  cout<<"This is what I am supposed to have: "<<ff_nom<<"    And this is what I have: "<<my_value<<endl;
      ...
      tau1Cand.clear();
      tau2Cand.clear();
      if (dont_double_count!=1){
         skip_me = -1;
         tau1Cand = getTau1CandFail(55, 2.1);
         if (tau1Cand.size()>0) skip_me=tau1Cand[0];
         tau2Cand = getTau2CandTight(45, 2.1, skip_me);
      }
	...
	if( (HLTEleMuX>>29&1) == 1 || (HLTEleMuX>>30&1) == 1 || (HLTEleMuX>>31&1) == 1 ){
		  ...
		  event_weight = event_weight*tauSFs.getTriggerScaleFactor(tauPt->at(tau2Cand[0]), tauEta->at(tau2Cand[0]), tauPhi->at(tau1Cand[0]), tauDecayMode->at(tau2Cand[0]));
                  ...
                  myMET.SetPtEtaPhiE(pfMET, 0, pfMETPhi, pfMET);
		  /*
                  inputs2[0] = tauPt->at(tau1Cand[0]);
                  inputs2[1] = tauPt->at(tau2Cand[0]);
                  inputs2[2] = tauDecayMode->at(tau2Cand[0]);
                  inputs2[3] = nJet;
                  inputs2[4] = VisMass_F(myTau1, myTau2);
                  inputs2[5] = 1;
                  inputs2[6] = 0;
                  inputs2[7] = 0;

                  ff_nom = ff2->value(inputs2);
                  // event_weight = event_weight * 0.5 * ff_nom;
		  */
                  float my_value = 0;
                  if (tauDecayMode->at(tau1Cand[0]) == 0) my_value = ff_mine_0->GetBinContent(ff_mine_0->GetXaxis()->FindBin( tauPt->at(tau1Cand[0]) ) );
      ...
      h_Events_level->SetBinContent(1, numberOfEvents*0.5);
      h_Events_level->SetBinContent(2, nMETFiltersPassed*0.5);
      h_Events_level->SetBinContent(3, nSingleTrgPassed*0.5);
      h_Events_level->SetBinContent(4, nGoodTau1Passed*0.5);
   ...
   std::cout<<std::setw(20) <<std::right <<"GoodTau1 Passed "<<nGoodTau1Passed<<std::endl;
   std::cout<<std::setw(20) <<std::right <<"GoodTau2 Passed "<<nGoodTau2Passed<<std::endl;
   std::cout<<std::setw(20) <<std::right <<"opp charge "<< nGoodPairPassed<<std::endl;
   std::cout<<std::setw(20) <<std::right <<"DeltaRPassed "<<nDeltaRPassed<<std::endl;
   std::cout<<std::setw(20) <<std::right <<"PassedThirdLepVeto "<<nPassedThirdLeptonVeto<<std::endl;
...
void Analyzer_tautau::BookHistos(const char* file2){
   ...
   Float_t MetBins[14]={0.0, 20, 40, 60, 80, 100, 120, 140, 160,180., 200, 300., 400., 500.0};
   Float_t TrMassBins[17]={0.0, 20, 40, 60, 80, 100, 120, 140, 160,180., 200, 225, 250,275, 300.0,400,500 };
...
std::vector<int> Analyzer_tautau::getTau1CandTight(double tauPtCut, double tauEtaCut){
...
std::vector<int> Analyzer_tautau::getTau1CandFail(double tauPtCut, double tauEtaCut){
   ...
   for (int iTau=0; iTau<nTau; iTau++){
      ...
      if( tauPt->at(iTau) > tauPtCut  && fabs( tauEta->at(iTau))< tauEtaCut && fabs(taudz->at(iTau))<0.2 )kinematic = true;
      if( tauByLooseMuonRejection3->at(iTau) == 1 && tauByMVA6VLooseElectronRejection->at(iTau) == 1) tauId = true;
      if( (tauByTightIsolationMVArun2v1DBoldDMwLT->at(iTau)!=1 && taubyVLooseIsolationMVArun2017v2DBoldDMwLT2017->at(iTau)==1)) tauIsolation = true;
      if( tauDecayMode->at(iTau)==0 || tauDecayMode->at(iTau)==1 || tauDecayMode->at(iTau)==10 ) decayModeCut = true;
...
std::vector<int> Analyzer_tautau::getTau2CandTight(double tauPtCut, double tauEtaCut, int skip_me){
...
std::vector<int> Analyzer_tautau::getTau2CandFail(double tauPtCut, double tauEtaCut, int skip_me){
...
bool Analyzer_tautau::thirdLeptonVeto(){
   ...
   for(int iEle=0; iEle < nEle;iEle++){
      ...
      if( (*elePt)[iEle] > 10.0  && fabs((*eleEta)[iEle])< 2.5 && fabs((*eleD0)[iEle]) < 0.045 && fabs((*eleDz)[iEle]) < 0.2 ) kinematicEle = true;

...
// Tue Jan  1 20:25:43 2019 by ROOT version 6.10/09
// from TTree EventTree/Event data (tag V09_04_09_00)
// found on file: /hdfs/store/user/gomber/MonoHiggs_2017_12April_AllMC_withtrigger/DYJetsToLL_M-50_TuneCP5_13TeV-madgraphMLM-pythia8/crab_DYJetsToLL_LO/181109_181451/0000/ggtree_mc_50.root
//////////////////////////////////////////////////////////

#ifndef Analyzer_tautau_h
#define Analyzer_tautau_h
...
public :
   ...
   TH1F *h_nVtx[25], *h_nEvents[25], *h_genWeight[25], *h_genHT[25];
   TH1F *h_tau1_En[25], *h_tau1_Pt[25], *h_tau1_eta[25], *h_tau1_phi[25];
   TH1F *h_tau2_En[25], *h_tau2_Pt[25], *h_tau2_eta[25], *h_tau2_phi[25];
   TH1F *h_pfMET[25], *h_pfMET_phi[25], *h_dPhi[25], *h_dR[25], *h_nJet[25], *h_leadingJetPt[25], *h_leadingJetEta[25];
   TH1F *h_Mt[25], *h_totTrMass[25], *h_VisibleMass[25], *h_HiggsPt[25], *h_tauIso[25], *h_tauMass[25], *h_tauDecayMode[25];
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Analyzer_tautau(const char* file1, const char* file2);
   virtual ~Analyzer_tautau();
   ...
   virtual void fillHistos(int histoNumber, double event_weight, int tau1Index, int tau2Index);
   //virtual vector<int> getTau1Cand(double tauPtCut, double tauEtaCut);
   //virtual vector<int> getTau2Cand(double tauPtCut, double tauETaCut, int skip_me);
   virtual vector<int> getTau1CandTight(double tauPtCut, double tauEtaCut);
   virtual vector<int> getTau1CandFail(double tauPtCut, double tauEtaCut);
   virtual vector<int> getTau2CandTight(double tauPtCut, double tauETaCut, int skip_me);
   virtual vector<int> getTau2CandFail(double tauPtCut, double tauETaCut, int skip_me);
   ...
   virtual double dR(int tau1Index, int tau2Index);
   virtual double DeltaPhi(double phi1, double phi2);
};

#endif

#ifdef Analyzer_tautau_cxx
Analyzer_tautau::Analyzer_tautau(const char* file1, const char* file2){

   TChain *chain = new TChain("ggNtuplizer/EventTree");

   // now we run over everything
   TString path = file1;
   ...
   while ((filename = (TSystemFile*)next()) && fileNumber > maxFiles){
      ...
      if(name.Contains(dataset)) chain->Add(FullPathInputFile);

      fileNumber++;
   }

   std::cout<<"All files added. >> MC "<<endl;
   std::cout<<"Initizlizing chain."<<endl;
   Init(chain);
   BookHistos(file2);
}

Analyzer_tautau::~Analyzer_tautau()
{
   if (!fChain) return;
...
Int_t Analyzer_tautau::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Analyzer_tautau::LoadTree(Long64_t entry)
{
...
void Analyzer_tautau::Init(TTree *tree)
{
...
void Analyzer_tautau::Show(Long64_t entry)
{
...
Int_t Analyzer_tautau::Cut(Long64_t entry)
{
...
#endif // #ifdef Analyzer_tautau_cxx

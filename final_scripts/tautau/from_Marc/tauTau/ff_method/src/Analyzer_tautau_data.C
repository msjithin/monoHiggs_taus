#define Analyzer_tautau_data_cxx
#include "Analyzer_tautau_data.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TH1F.h"
...
int main(int argc, const char* argv[]){
   ...
   Analyzer_tautau_data t(argv[1], argv[2]);
   t.Loop(maxEvents, reportEvery, SampleName);
   return 0;
}


void Analyzer_tautau_data::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
   if (fChain == 0) return;
...
void Analyzer_tautau_data::BookHistos(const char* file2){
...
void Analyzer_tautau_data::fillHistos(int histoNumber, double event_weight, int tau1Index, int tau2Index){
...
std::vector<int> Analyzer_tautau_data::getTau1Cand(double tauPtCut, double tauEtaCut){
...
std::vector<int> Analyzer_tautau_data::getTau2Cand(double tauPtCut, double tauEtaCut, int skip_me){
...
bool Analyzer_tautau_data::thirdLeptonVeto(){
   ...
   for(int iMu=0; iMu<nMu; iMu++){
      ...
      if( (*muPt)[iMu] > 10.0  && fabs((*muEta)[iMu])< 2.4 && fabs((*muD0)[iMu]) < 0.045 && fabs((*muDz)[iMu]) < 0.2 ) kinematicMu = true;
...
bool Analyzer_tautau_data::passBjetVeto(){
   int tmpCand=0;
   bool veto = true;
...
bool Analyzer_tautau_data::check_jets(){
  //int tmpCand=0;
...
double Analyzer_tautau_data::prefiring_weight(TH2F *prefiring_photon, TH2F *prefiring_jet){
    double weight = 1.0;
...
float Analyzer_tautau_data::TMass_F(float LepPt, float LepPhi, float met, float metPhi){
   return sqrt(pow(LepPt + met, 2) - pow(LepPt* cos(LepPhi) + met * cos(metPhi), 2) - pow(LepPt * sin(LepPhi) + met * sin(metPhi), 2));
}


float Analyzer_tautau_data::TotTMass_F(TLorentzVector a, TLorentzVector b, TLorentzVector met){
   float totalTMass = ( a + b + met).M();
   return totalTMass;
}

float Analyzer_tautau_data::VisMass_F(TLorentzVector a, TLorentzVector b){
  float visibleMass = (a + b).M();
  return visibleMass;
}

float Analyzer_tautau_data::pTvecsum_F(float pt1, float pt2, float phi1, float phi2){
   float pt_vecSum = sqrt( pow(pt1*cos(phi1) + pt2*cos(phi2), 2) + pow(pt1*sin(phi1) + pt2*sin(phi2), 2));
  return pt_vecSum;
}


double Analyzer_tautau_data::dR(int tau1_index, int tau2_index){
...
double Analyzer_tautau_data::DeltaPhi(double phi1, double phi2){
   double pi = TMath::Pi();

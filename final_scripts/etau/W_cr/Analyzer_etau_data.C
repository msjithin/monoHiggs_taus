...
std::vector<int> Analyzer_etau_data::getRelaxedEleCand(double elePtCut, double eleEtaCut){
  ...
  for(int iEle=0;iEle<nEle;iEle++)
    {
      ...
      if( relEleIso < 0.30 ) relative_iso = true;
      if(electronId && kinematic && relative_iso){
        tmpCand.push_back(iEle);
      }
    }
...
std::vector<int> Analyzer_etau_data::getRelaxedTauCand(double tauPtCut, double tauEtaCut){

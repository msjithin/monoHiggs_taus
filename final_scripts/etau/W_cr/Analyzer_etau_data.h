...
#ifndef Analyzer_etau_data_h
#define Analyzer_etau_data_h
...
public :
   ...
   TH1F *h_electron_En[25], *h_electron_Pt[25], *h_electron_eta[25], *h_electron_SCEta[25], *h_electron_phi[25],  *h_electron_SCPhi[25],  *h_electron_IDbit[25];
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   //   Analyzer_etau_data(TTree *tree=0);
   Analyzer_etau_data(const char* file1, const char* file2);
   virtual ~Analyzer_etau_data();
   ...
   virtual void fillHistos(int histoNumber, double event_weight, int eleIndex, int tauIndex);
   ...
   virtual vector<int> getRelaxedEleCand(double pt, double eta);
   virtual vector<int> getRelaxedTauCand(double pt, double eta);
...
#ifdef Analyzer_etau_data_cxx
Analyzer_etau_data::Analyzer_etau_data(const char* file1, const char* file2)
{
...
Analyzer_etau_data::~Analyzer_etau_data()
{
   if (!fChain) return;
...
Int_t Analyzer_etau_data::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Analyzer_etau_data::LoadTree(Long64_t entry)
{
...
void Analyzer_etau_data::Init(TChain *tree)
{
...
Bool_t Analyzer_etau_data::Notify()
{
...
void Analyzer_etau_data::Show(Long64_t entry)
{
...
Int_t Analyzer_etau_data::Cut(Long64_t entry)
{
...
#endif // #ifdef Analyzer_etau_data_cxx

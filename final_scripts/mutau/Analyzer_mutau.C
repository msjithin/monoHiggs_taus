   ...
   Long64_t nentriesToCheck = nentries;
	...
	for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
	{
	  ...
	  */
	  ...
	  //h_Events_level->SetBinContent(4, nL1PrefiringPassed);
	  ...
	  //h_Events_level->SetBinContent(13, nPasstottrmass);
	  // 
...
double Analyzer_mutau::prefiring_weight(TH2F* prefiring_photon, TH2F* prefiring_jet)
{
  ...
  //for (int x : tmpCand_jet ) std::cout<<"tmpCand_jets elements  "<< x<< endl;
  weight = (1-map_val_ele)*(1-map_val_jet);
  //if (weight != 1) std::cout<<"weight = " << (1-map_val_ele)<< " and "<<(1-map_val_jet) << endl;

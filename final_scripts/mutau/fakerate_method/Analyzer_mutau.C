...
void Analyzer_mutau::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
   ...
   double nHLTPassed,n_eventWeight, nSingleTrgPassed, nGoodMuonPassed, nElectronPtPassed, nGoodTauPassed, nTauPtPassed,numberOfEvents,nMETPassed, nDPhiPassed, nqcdden,nqcdnum, nMETFiltersPassed,nLeptonVetoPassed,nPassedBjetVeto,nNoisyCrystals,nDPhiJetMETPassed, nGoodMuTauPassed,nDeltaRPassed,nPassedThirdLepVeto, nPassedHiggsPtcut, nPassedVisibleMasscut, nPassedMETcut, nFinal_afterSelections, nGoodMuonPassed_qcd, nGoodTauPassed_qcd, nGoodMuTauPassed_qcd, nDeltaRPassed_qcd, nPassedThirdLepVeto_qcd,nPassedBjetVeto_qcd, nPassedHiggsPtcut_qcd, nPassedVisibleMasscut_qcd, nPassedMETcut_qcd, nFinal_afterSelections_qcd, nMtPassed,nMtPassed_qcd,nPasstottrmass, nPassJetsSelection,nL1PrefiringPassed;
   nHLTPassed = n_eventWeight = nSingleTrgPassed = nGoodMuonPassed = nElectronPtPassed = nGoodTauPassed = nTauPtPassed= numberOfEvents = nMETPassed = nDPhiPassed = nqcdden= nqcdnum=nMETFiltersPassed= nLeptonVetoPassed=nPassedBjetVeto=nNoisyCrystals=nDPhiJetMETPassed= nGoodMuTauPassed = nDeltaRPassed= nPassedThirdLepVeto=nPassedHiggsPtcut=nPassedVisibleMasscut=nPassedMETcut=nFinal_afterSelections=nGoodMuonPassed_qcd=nGoodTauPassed_qcd=nGoodMuTauPassed_qcd=nDeltaRPassed_qcd=nPassedThirdLepVeto_qcd=nPassedBjetVeto_qcd=nPassedHiggsPtcut_qcd=nPassedVisibleMasscut_qcd=nPassedMETcut_qcd=nFinal_afterSelections_qcd=nMtPassed=nMtPassed_qcd=nPassJetsSelection=nPasstottrmass=nL1PrefiringPassed=0;
		      ...
		      if( tauCand.size() >0 )
			{
			  ...
			  */
			  ...
			  if ( tauDecayMode->at(tauCand[0])==10)
			      ...
			      //std::cout<<"tau_FR  "<< tau_FR_10 <<std::endl;
						      ...
						      if( (pfMET > 105) ) 
							{
							  ...
							  if (tot_tr_mass > 260)
							      ...
							      if (check_jets()==false)
								{
								  ...
								  fillHistos(9,event_weight,muCand[0], tauCand[0]);
								}
							    }
	...
	for(int i=0; i<20; i++)
	  {
		...
		h_muonIso[i]= new TH1F(("Muon_iso"+histname).c_str(),"Muon_iso",25,0.0,0.5);h_muonIso[i]->Sumw2();
		h_tauIso[i]= new TH1F(("Tau_iso"+histname).c_str(),"Tau_iso", 12, 0.0, 1.2);h_tauIso[i]->Sumw2();
		h_tauMass[i]= new TH1F(("Tau_mass"+histname).c_str(),"Tau_mass", 30, 0.0, 3.0);h_tauMass[i]->Sumw2();
...
void Analyzer_mutau::fillHistos(int histoNumber, double event_weight,int muIndex, int tauIndex)
{
	...
	//*********** fill rest  ***********
		...
		h_tauIso[histoNumber]->Fill(taubyIsolationMVArun2017v2DBoldDMwLTraw2017->at(tauIndex),event_weight);

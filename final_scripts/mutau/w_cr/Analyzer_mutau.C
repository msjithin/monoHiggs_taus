...
void Analyzer_mutau::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
   ...
   double nHLTPassed,n_eventWeight, nSingleTrgPassed, nGoodMuonPassed, nElectronPtPassed, nGoodTauPassed, nTauPtPassed,numberOfEvents,nMETPassed, nDPhiPassed, nqcdden,nqcdnum, nMETFiltersPassed,nLeptonVetoPassed,nPassedBjetVeto,nNoisyCrystals,nDPhiJetMETPassed, nGoodMuTauPassed,nDeltaRPassed,nPassedThirdLepVeto, nPassedHiggsPtcut, nPassedVisibleMasscut, nPassedMETcut, nFinal_afterSelections, nGoodMuonPassed_qcd, nGoodTauPassed_qcd, nGoodMuTauPassed_qcd, nDeltaRPassed_qcd, nPassedThirdLepVeto_qcd,nPassedBjetVeto_qcd, nPassedHiggsPtcut_qcd, nPassedVisibleMasscut_qcd, nPassedMETcut_qcd, nFinal_afterSelections_qcd, nMtPassed;
   nHLTPassed = n_eventWeight = nSingleTrgPassed = nGoodMuonPassed = nElectronPtPassed = nGoodTauPassed = nTauPtPassed= numberOfEvents = nMETPassed = nDPhiPassed = nqcdden= nqcdnum=nMETFiltersPassed= nLeptonVetoPassed=nPassedBjetVeto=nNoisyCrystals=nDPhiJetMETPassed= nGoodMuTauPassed = nDeltaRPassed= nPassedThirdLepVeto=nPassedHiggsPtcut=nPassedVisibleMasscut=nPassedMETcut=nFinal_afterSelections=nGoodMuonPassed_qcd=nGoodTauPassed_qcd=nGoodMuTauPassed_qcd=nDeltaRPassed_qcd=nPassedThirdLepVeto_qcd=nPassedBjetVeto_qcd=nPassedHiggsPtcut_qcd=nPassedVisibleMasscut_qcd=nPassedMETcut_qcd=nFinal_afterSelections_qcd=nMtPassed=0;
   ...
   Long64_t nentriesToCheck = nentries;
	...
	for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
	{
	  ...
	  relaxedMuCand = getRelaxedMuCand(30, 2.4);
	  relaxedTauCand = getRelaxedTauCand(20, 2.3);
	      ...
	      if(HLTEleMuX>>19&1 == 1 ) // Single muon trigger
		{
		  ...
		  if(muCand.size() >0) 
		      ...
		      if( tauCand.size() >0 )
			{
			  ...
			  if (muPt->at(muCand[0]) <=120)
			    {
			      ...
			      std::cout<<"sf_muID =  "<< sf_muID<<std::endl;
			      ...
			      std::cout<<"sf_IsoEff =  "<< sf_IsoEff<<std::endl;
			    }
			  ...
			  std::cout<<"sf_muTrg =  "<< sf_muTrg<<std::endl;
			  ...
			  if( (muCharge->at(muCand[0]))*(tauCharge->at(tauCand[0])) < 0 ) // opposite cgharge condition
			    {
			      std::cout<<"This works! Charge criteria" <<endl;
			      //h_mupt_2->Fill(muPt->at(muCand[0]), event_weight);
	...
	for(int i=0; i<20; i++)
	  {
		...
		h_muonIso[i]= new TH1F(("Muon iso"+histname).c_str(),"Muon iso",40,0.0,1.0);h_muonIso[i]->Sumw2();
  ...
  for(int iMu=0;iMu<nMu;iMu++)
    {
      ...
      if( (*muPt)[iMu] > muPtCut  && fabs((*muEta)[iMu])< muEtaCut && muDz->at(iMu) < 0.2 && muD0->at(iMu)<0.45) kinematic = true;
      bool muonId =false;
      if( muIDbit->at(iMu)>>2&1==1) muonId =true;
...
std::vector<int> Analyzer_mutau::getRelaxedMuCand(double muPtCut, double muEtaCut){
  std::vector<int> tmpCand;
  tmpCand.clear();
  //Loop over muons                                                                     
  for(int iMu=0;iMu<nMu;iMu++)
...
std::vector<int> Analyzer_mutau::getRelaxedTauCand(double tauPtCut, double tauEtaCut){
  std::vector<int> tmpCand;
  tmpCand.clear();
  //Loop over taus      
  for(int iTau=0;iTau<nTau;iTau++)

...
void Analyzer_etau::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
   ...
   bool debug=true;  double netWeight = 1.0;
   ...
   std::vector<int> relaxedEleCand;
   relaxedEleCand.clear();

   std::vector<int> relaxedTauCand;
   relaxedTauCand.clear();
   
   std::vector<int> particleCand;
   particleCand.clear();
   ...
   Long64_t nentriesToCheck = nentries;
	...
	for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
	{
	  ...
	  //std::cout<< "scaleFactor_1 = "<<scaleFactor_1 <<endl;
	  //event_weight*=sf_tauID*sf_Zvtx;
	  ...
	  //if(sample== "DYJetsToLL" or sample== "TTJets" or sample== "WJetsToLNu_2J_0" or sample== "WJetsToLNu_2J_1" or sample== "WZTo1L1Nu2Q" or sample== "WZTo2L2Q" or sample== "WZTo3LNu" or sample== "ZZTo2L2Q"){
	    
	  //if ( jentry%reportEvery == 0) std::cout<< "Found FXFX!" <<endl;
	  //genWeight > 0.0 ?  event_weight = 1.0 : event_weight = 0.0;
	  //}
	  if (jentry%reportEvery == 0){ std::cout<<"Sample name ="<< sample <<endl;}
	  
	  if(sample== "WJetsToLNu" or sample== "WJetsToLNu_1")
	    {
	      if ( jentry%reportEvery == 0)std::cout<< "******************** Found Wjets_2J **************" <<endl;
	  ...
	  if(metFilters==0) 
	    {
	      ...
	      if (jentry%reportEvery == 0){ std::cout<<"This works! Met filters" <<endl;}
	      
	      
	      if(HLTEleMuX>>3&1 == 1) // Single electron trigger
		{
		  nSingleTrgPassed+=event_weight;
		  if (jentry%1000 == 0) std::cout<<"This works! Single electron trigger" <<endl;
		  ...
		  if(eleCand.size() >0) 
		    {
		      ...
		      if (jentry%1000 == 0) std::cout<<"This works! eleCand" <<endl;
		      ...
		      if( tauCand.size() >0 )
			{
			  ...
			  std::cout<<"event_weight (PE) =  "<< event_weight<<std::endl;
			  
			  eleRecoSF_corr=h_eleRecoSF_highpt->GetBinContent(h_eleRecoSF_highpt->GetXaxis()->FindBin(eleEta->at(eleCand[0])),h_eleRecoSF_highpt->GetYaxis()->FindBin(elePt->at(eleCand[0])));
			  std::cout<<"eleRecoSF_corr =  "<< eleRecoSF_corr<<std::endl;
			  eleEffSF_corr=h_eleIDSF->GetBinContent(h_eleIDSF->GetXaxis()->FindBin(eleEta->at(eleCand[0])),h_eleIDSF->GetYaxis()->FindBin(elePt->at(eleCand[0])));
			  std::cout<<"eleEffSF_corr =  "<< eleEffSF_corr<<std::endl;
			  eletrgsf = EletriggerSF(elePt->at(eleCand[0]), eleEta->at(eleCand[0]));
			  event_weight=event_weight*eleRecoSF_corr*eleEffSF_corr*sf_Zvtx*eletrgsf;
			  std::cout<<"event_weight (AFTER SF) =  "<< event_weight<<std::endl;
			  netWeight = event_weight;
			  if( (eleCharge->at(eleCand[0]))*(tauCharge->at(tauCand[0])) < 0 ) // opposite charge condition
			    {
			      if (jentry%5 == 0) std::cout<<"This works! Charge criteria" <<endl;
			      ...
			      if(deltaR > 0.3 )
				{
				  ...
				  if( thirdLeptonVeto()==true )
				  {
				    ...
				    if (jentry%5 == 0) std::cout<<"This works! third lepon veto" <<endl;
				    fillHistos(2,event_weight,eleCand[0], tauCand[0]);
				    if( passBjetVeto() == true)
				      {
					nPassedBjetVeto+=event_weight;
					if (jentry%5 == 0) std::cout<<"This works! bJetVeto" <<endl;
					fillHistos(3,event_weight,eleCand[0], tauCand[0]);
					if (jentry%5 == 0) std::cout<<"This works! bJetVeto **** after fill" <<endl;
						...
						if( (pfMET > 105) )
						  {
						    ...
						    if (jentry%5 == 0) std::cout<<"******************* This works! All selections passed ***************" <<endl;
		  ...
		  if (relaxedEleCand.size() > 0 )
		    {
		      nGoodElectronPassed_qcd+=event_weight;
		      if ( relaxedTauCand.size() > 0)
			{
			  nGoodTauPassed_qcd+=event_weight;
			  //std::cout<<"This works! Qcd starting" <<endl;
			  if( (eleCharge->at(relaxedEleCand[0]))*(tauCharge->at(relaxedTauCand[0])) > 0 ) // same charge condition for Qcd
			    {
			      //std::cout<<"This works! Qcd Charge criteria" <<endl;
			      //std::cout<<"event_weight (PE) for qcd  =  "<< event_weight<<std::endl;
			      
			      eleRecoSF_corr=h_eleRecoSF_highpt->GetBinContent(h_eleRecoSF_highpt->GetXaxis()->FindBin(eleEta->at(relaxedEleCand[0])),h_eleRecoSF_highpt->GetYaxis()->FindBin(elePt->at(relaxedEleCand[0])));
			      std::cout<<"eleRecoSF_corr =  "<< eleRecoSF_corr<<std::endl;
			      eleEffSF_corr=h_eleIDSF->GetBinContent(h_eleIDSF->GetXaxis()->FindBin(eleEta->at(relaxedEleCand[0])),h_eleIDSF->GetYaxis()->FindBin(elePt->at(relaxedEleCand[0])));
			      std::cout<<"eleEffSF_corr =  "<< eleEffSF_corr<<std::endl;
			      event_weight=event_weight*eleRecoSF_corr*eleEffSF_corr*sf_Zvtx;
			      std::cout<<"event_weight (AFTER SF) for qcd =  "<< event_weight<<std::endl;
			      
			      
			      nGoodETauPassed_qcd+=event_weight;
			      fillHistos(10,event_weight,relaxedEleCand[0], relaxedTauCand[0]);
			      
			      double deltaR = dR(relaxedEleCand[0],relaxedTauCand[0]);
			      if(deltaR > 0.3 )
				{
				  //std::cout<<"This works! dR cut" <<endl;
				  nDeltaRPassed_qcd+=event_weight;
				  fillHistos(11,event_weight,relaxedEleCand[0], relaxedTauCand[0]);
				  //std::cout<<"event_weight (AFTER SF 3) =  "<< event_weight<<std::endl;
				  
				  if( thirdLeptonVeto()==true )
				    {
				      nPassedThirdLepVeto_qcd+=event_weight;
				      //std::cout<<"This works! third lepon veto" <<endl;
				      fillHistos(12,event_weight,relaxedEleCand[0], relaxedTauCand[0]);
				      if( passBjetVeto() == true)
					{
					  nPassedBjetVeto_qcd+=event_weight;
					  //std::cout<<"This works! bJetVeto" <<endl;
					  fillHistos(13,event_weight,relaxedEleCand[0], relaxedTauCand[0]);
					  if (jentry%5 == 0) std::cout<<"This works! bJetVeto **** after fill" <<endl;
					  
					  
					  double HiggsPt = pTvecsum_F(elePt->at(relaxedEleCand[0]),tauPt->at(relaxedTauCand[0]),elePhi->at(relaxedEleCand[0]),tauPhi->at(relaxedTauCand[0]));
					  if( (HiggsPt>65) )
					    {
					      nPassedHiggsPtcut_qcd+=event_weight;
					      fillHistos(14,event_weight,relaxedEleCand[0], relaxedTauCand[0]);
					      
					      TLorentzVector myTau;
					      myTau.SetPtEtaPhiE(tauPt->at(relaxedTauCand[0]),tauEta->at(relaxedTauCand[0]),tauPhi->at(relaxedTauCand[0]), tauEnergy->at(relaxedTauCand[0]));
					      TLorentzVector myEle;
					      myEle.SetPtEtaPhiE(elePt->at(relaxedEleCand[0]),eleEta->at(relaxedEleCand[0]),elePhi->at(relaxedEleCand[0]), eleEn->at(relaxedEleCand[0]));
					      double visMass_etau = VisMass_F(myTau, myEle);
					      
					      if( (visMass_etau < 125) ) 
						{
						  nPassedVisibleMasscut_qcd+=event_weight;
						  fillHistos(15,event_weight,relaxedEleCand[0], relaxedTauCand[0]);
						  
						  
						  if( (pfMET > 105) )
						    {
						      nPassedMETcut_qcd+=event_weight;
						      fillHistos(16,event_weight,relaxedEleCand[0], relaxedTauCand[0]);
						      
						      if (jentry%5 == 0) std::cout<<"******************* This works! All selections passed ***************" <<endl;
						      nFinal_afterSelections_qcd+=event_weight;
						    }
						}
					    }
					}
	  ...
	  h_Events_level->SetBinContent(11, nPassedVisibleMasscut); 
	  ...
	  h_Events_level_qcd->SetBinContent(1, nInspected_genWeighted);
	  h_Events_level_qcd->SetBinContent(2, nMETFiltersPassed);
          h_Events_level_qcd->SetBinContent(3, nSingleTrgPassed);          
	  h_Events_level_qcd->SetBinContent(4, nGoodElectronPassed_qcd);
	  h_Events_level_qcd->SetBinContent(5, nGoodTauPassed_qcd);	  
          h_Events_level_qcd->SetBinContent(6, nGoodETauPassed_qcd);
          h_Events_level_qcd->SetBinContent(7, nDeltaRPassed_qcd);
          h_Events_level_qcd->SetBinContent(8, nPassedThirdLepVeto_qcd); 
	  h_Events_level_qcd->SetBinContent(9, nPassedBjetVeto_qcd);
	  h_Events_level_qcd->SetBinContent(10, nPassedHiggsPtcut_qcd); 
	  h_Events_level_qcd->SetBinContent(11, nPassedVisibleMasscut_qcd); 
	  h_Events_level_qcd->SetBinContent(12, nPassedMETcut_qcd); 
	  //          h_Events_level->SetBinContent(13, nFinal_afterSelections);
          h_Cutflow_qcd->SetBinContent(1, nGoodElectronPassed_qcd);
          h_Cutflow_qcd->SetBinContent(2, nGoodTauPassed_qcd);
          h_Cutflow_qcd->SetBinContent(3, nGoodETauPassed_qcd);
          h_Cutflow_qcd->SetBinContent(4, nDeltaRPassed_qcd);
          h_Cutflow_qcd->SetBinContent(5, nPassedThirdLepVeto_qcd);
          h_Cutflow_qcd->SetBinContent(6, nPassedBjetVeto_qcd);
	...
	std::cout<<"*******************************************"<<std::endl;
	std::cout<<std::setw(20) <<std::right <<"GoodElectronPassed_qcd "<<nGoodElectronPassed_qcd<<std::endl;
	std::cout<<std::setw(20) <<std::right <<"GoodTauPassed_qcd "<<nGoodTauPassed_qcd<<std::endl;
	std::cout<<std::setw(20) <<std::right <<"opp charge_qcd "<<nGoodETauPassed_qcd<<std::endl;
	std::cout<<std::setw(20) <<std::right <<"DeltaRPassed_qcd "<<nDeltaRPassed_qcd<<std::endl;
	std::cout<<std::setw(20) <<std::right <<"PassedThirdLepVeto_qcd "<<nPassedThirdLepVeto_qcd<<std::endl;
	std::cout<<std::setw(20) <<std::right <<"PassedBjetVeto_qcd "<<nPassedBjetVeto_qcd<<std::endl;
	std::cout<<"*******************************************"<<std::endl;
...
std::vector<int> Analyzer_etau::getRelaxedEleCand(double elePtCut, double eleEtaCut){
...
std::vector<int> Analyzer_etau::getRelaxedTauCand(double tauPtCut, double tauEtaCut){

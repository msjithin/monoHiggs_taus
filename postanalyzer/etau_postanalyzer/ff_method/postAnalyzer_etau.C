...
void make_hist(string input_file, string output_file, string histname_string, string sample_name, string name_ ,  Double_t weight_lumi , bool isNLO, double ngen_, double xsec)
{
  ...
  */
  ...
  else if(histname == "ffEvents"){
    TH1F* histo_events_level = (TH1F*)((TH1F*)file_input->Get("ffEvents"))->Clone(TString(name+"_"+histname));
    // cout<<"3. updating file for "<< histname<<endl;
    histo_events_level->Scale(net_weight);
    histo_events_level->Write();
  }
...
int main(int argc, char** argv)
{
  ...
  */
  ...
  else if (sample=="ZTTinc"){ xs=LOtoNNLO_DY*5765.4; weight=2.456509;}
  else if (sample=="ZTT1jet"){ xs=LOtoNNLO_DY*1012.5; weight=0.893174237;}
  else if (sample=="ZTT2jet"){ xs=LOtoNNLO_DY*332.8; weight=0.989793913;}
  else if (sample=="ZTT3jet"){ xs=LOtoNNLO_DY*101.8; weight=1.560451682;}
  else if (sample=="ZTT4jet"){ xs=LOtoNNLO_DY*54.8; weight=0.487844385;}
...
//  plotnames.push_back(TString(""));
  ...
  histnames.push_back("ffEvents");
  leg_xoffsets.push_back(0.);
  leg_yoffsets.push_back(0.);
  xaxis_titles.push_back(TString("ffEvents"));
  plotnames.push_back(TString("ffEvents"));
...
*/
  ...
  */
  ...
  TDirectory *ffEvents = f_output->mkdir("ffEvents"); 
  ...
  make_hist(input, output, "ffEvents", sample ,name,weight, true, ngen , xs);
